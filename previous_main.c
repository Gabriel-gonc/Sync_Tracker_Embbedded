...
static void wait_message(char* msg_to_wait, char* reply);
...
static void wait_message(char* msg_to_wait, char* reply)
{
    while(1)
    {
        ...
        {
            if (strncmp(udp_receive_buffer, msg_to_wait, strlen(msg_to_wait)) == 0)
            {
                int len = snprintf(udp_send_buffer, sizeof(udp_send_buffer), reply);
