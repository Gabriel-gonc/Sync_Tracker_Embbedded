#ifndef PROJECT_TYPES_H
#define PROJECT_TYPES_H

/***********************************************************
 * Project Defines
***********************************************************/
/** @brief Pinout */
#define ELETRIC_GRID_PIN GPIO_NUM_4
#define SENSOR_PIN GPIO_NUM_5
#define BREAKER_PIN GPIO_NUM_19

/** @brief Project Constants */
#define POLES_PAIRS (1) // If the machine has 1 magnet for each pole pair this value must be 1
#define GRID_FREQUENCY (60)
#define SENSOR_FREQUENCY (GRID_FREQUENCY / POLES_PAIRS)
#define SAMPLE_DURATION_SEC (1)
#define NUM_CYCLES_DIFF_PULSE (SENSOR_FREQUENCY * SAMPLE_DURATION_SEC) 
#define GRID_WINDOW_FILTER (13000) 
#define SENSOR_WINDOW_FILTER (GRID_WINDOW_FILTER * POLES_PAIRS)
#define GRID_FREQUENCY_BUFFER_SIZE (GRID_FREQUENCY + 1)
#define SENSOR_FREQUENCY_BUFFER_SIZE (SENSOR_FREQUENCY + 1)

/** @brief UDP Constants */
#define UDP_RX_BUFFER_SIZE 50
#define UDP_TX_BUFFER_SIZE GRID_FREQUENCY_BUFFER_SIZE * 6

/** @brief UDP Messages and Commands */
#define MSG_SYNC "SYNC"
#define CMD_ACK "ACK"
#define CMD_NACK "NACK"
#define CMD_FAULT "FAULT"
#define MSG_TIME_DIFF_MON "TIME_DIFF_MONITORING"
#define MSG_FNSH_TIME_DIFF_MON "FINISH_TIME_DIFF_MONITORING"
#define MSG_OP "OPERATIONAL"
#define MSG_FREQ "FREQUENCY"
#define MSG_FNSH_OP "FINISH_OPERATIONAL"
#define MSG_FNSH_FREQ "FINISH_FREQUENCY"

/***********************************************************
 * Project Types
***********************************************************/
/** @brief Enum system events. */
typedef enum {
    SYSTEM_WIFI_CONNECTED,
    SYSTEM_WIFI_DISCONNECTED,
    SYSTEM_UDP_CREATED,
} system_event_t;

/** @brief System States */
typedef enum {
    EXIT_CRITERIA = -1,
    MSG_RECEIVED,
    NO_MSG_RECEIVED,
    STATE_IDLE,
    STATE_CONNECTED,
    STATE_TIME_DIFF_MONITORING,
    STATE_OPERATIONAL,
    STATE_FREQUENCY_MONITORING,
    STATE_ERROR,
} system_state_t;

/** @brief System callback. */
typedef void (*system_callback_t)(system_event_t event);

#endif // PROJECT_TYPES_H